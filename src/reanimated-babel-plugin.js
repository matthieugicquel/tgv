...
const objectHooks = new Set(['useAnimatedGestureHandler', 'useAnimatedScrollHandler']);
...
const gestureHandlerCallbackMethods = new Set(['onBegan', 'onStart', 'onEnd', 'onUpdate']);
...
function makeWorklet(t, fun, fileName) {
  ...
  if (fun.parent && fun.parent.callee && fun.parent.callee.name === 'useAnimatedStyle') {
  ...
  if (options && options.optFlags) {
    statements.push(
      t.expressionStatement(
        t.assignmentExpression(
          '=',
          t.memberExpression(privateFunctionId, t.identifier('__optimalization'), false),
  ...
  statements.push(
    t.expressionStatement(
      t.callExpression(
        t.memberExpression(t.identifier('global'), t.identifier('__reanimatedWorkletInit'), false),
        [privateFunctionId]
      )
    )
  );
    ...
    DirectiveLiteral(path) {
      ...
      if (value === 'worklet' && path.getFunctionParent() === fun) {
        ...
        if (
          ...
          directives.some(
            directive =>
              t.isDirectiveLiteral(directive.value) && directive.value.value === 'worklet'
          )
        ) {
...
function processIfGestureHandlerEventCallbackFunctionNode(t, fun, fileName) {
  ...
  */
  ...
  if (t.isCallExpression(fun.parent) && isGestureObjectEventCallbackMethod(t, fun.parent.callee)) {
    processWorkletFunction(t, fun, fileName);
  }
}
...
function processWorklets(t, path, fileName) {
  ...
  if (objectHooks.has(name) && path.get('arguments.0').type === 'ObjectExpression') {
